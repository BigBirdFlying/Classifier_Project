...
int H_LB_XinJiangZhongHe_1X_ImageProcess::K_ImageProcessInterfaceFunc(int iIDMark,unsigned char* pImgData,int iImgWidth,int iImgHeight,ConfigurationInfo& tConfigurationInfo,PH_DefectSet pH_DefectSet,bool IsDebug)
{
	...
	if(Left_Edge<tConfigurationInfo.iLeftEdgeLimit[tConfigurationInfo.tSteelInfo.iCamera])
	{
		Left_Edge=tConfigurationInfo.iLeftEdgeLimit[tConfigurationInfo.tSteelInfo.iCamera];
	}
	if(Right_Edge>tConfigurationInfo.iRightEdgeLimit[tConfigurationInfo.tSteelInfo.iCamera])
	{
		Right_Edge=tConfigurationInfo.iRightEdgeLimit[tConfigurationInfo.tSteelInfo.iCamera];
	}
	...
	if(Right_Edge-Left_Edge>iImgWidth*0.2)
	{
		...
		if(!bIsValidJudge)
		{
			Defects_ColdSteel_OpenCV(cvSrcImg_P2D,cvSrcImg_P2D_GCG,Left_Edge/2,Right_Edge/2,2,tConfigurationInfo,pH_DefectSet);
		}		
	}

	Defects_Period_OpenCV(pH_DefectSet,0.25,100,9,tConfigurationInfo.tSteelInfo.iImgIndex);
...
void H_LB_XinJiangZhongHe_1X_ImageProcess::Defects_ColdSteel_OpenCV(const cv::Mat& SrcImg,const cv::Mat& SrcImg_GCG,int Left_Edge,int Right_Edge,int iZoom,ConfigurationInfo& tConfigurationInfo,PH_DefectSet pH_DefectSet) 
{
	...
	{ 
		...
		//面积限制
		if(area<tConfigurationInfo.iAreaThreshold)//500
		{
			continue;
		}
		area_sum+=area;
