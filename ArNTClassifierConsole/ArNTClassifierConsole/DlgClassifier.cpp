...
BOOL DlgClassifier::OnInitDialog()
{
	...
	//特征列表
	m_listBoxFeatFrom.AddString(L"LBP特征");
	m_listBoxFeatFrom.AddString(L"Sobel特征");
	m_listBoxFeatFrom.AddString(L"HOG特征");
	m_listBoxFeatFrom.AddString(L"Haar特征");
	m_listBoxFeatFrom.AddString(L"矩特征");
	m_listBoxFeatFrom.AddString(L"原始图");
	m_listBoxFeatFrom.AddString(L"直方图");
	m_listBoxFeatFrom.AddString(L"灰度共生矩");
	//分类器列表
	m_tabCtrlClassifier.InsertItem(0, _T("BP分类器"));  
	m_tabCtrlClassifier.InsertItem(1, _T("SVM分类器")); 
	m_tabCtrlClassifier.InsertItem(2, _T("TREE分类器")); 
	m_tabCtrlClassifier.InsertItem(3, _T("Boosting分类器")); 
	m_tabCtrlClassifier.InsertItem(4, _T("CNN分类器")); 
...
bool DlgClassifier::CheckSampleFilePath()
{
	...
	while (bFind)
	{
		...
		if (finder.IsDirectory())
		{
			...
			bool isEffect=false;
			for(int i=0;i<CArNTClassifierConsoleDlg::m_ConfigClassSet.iItemNum;i++)
			{
				USES_CONVERSION;
				CString strClassName = A2T(CArNTClassifierConsoleDlg::m_ConfigClassSet.Items[i].strClassName);
	...
	while (bFind)
	{
		...
		if (finder.IsDirectory())
		{
			...
			for(int i=0;i<CArNTClassifierConsoleDlg::m_ConfigClassSet.iItemNum;i++)
			{
				...
				if(strFileName.Compare(strClassName)==0)
				{
					iInClass=CArNTClassifierConsoleDlg::m_ConfigClassSet.Items[i].iInternalClassNo;
				}
			}			
...
void DlgClassifier::ShowSampleImageToList()
{
	...
	{
		USES_CONVERSION;
		CString strClassName = A2T(CArNTClassifierConsoleDlg::m_ConfigClassSet.Items[m_iExterToInter[i]].strClassName);
		CString strNum;
...
void DlgClassifier::OnBnClickedButtonTrainAndTestClassifier()
{
	...
	case 0:
		...
		WideCharToMultiByte(CP_ACP, 0, strClassifierFileName, -1, m_ClassifierParam.fileName, iLength, NULL, NULL);		
		//ImgProcessClassifier::Classifier_BP(trainData,trainLabels,m_ClassIndexNum,m_ClassifierParam);
		H_ClassifierTrain("BP分类器",trainData,trainLabels,m_ClassIndexNum,m_ClassifierParam);
		strInfo.Format(L"BP分类器：隐藏层为%d，迭代次数为%d，结果精度为%f，存储文件名为%s",m_ClassifierParam.iHideNode,m_ClassifierParam.iMaxIter,m_ClassifierParam.fEpsilon,strClassifierFileName);
		m_ListShowInfo.InsertString(g_lInsertIndex++,strInfo);
		m_ListShowInfo.InsertString(g_lInsertIndex++,L"测试中...");
		H_ClassifierPredict("BP分类器",testData,testLabels,perLabels,m_ClassIndexNum,m_ClassifierParam);
		break;
	case 1:
		m_dlgClassifierSVM.GetDlgItem(IDC_EDIT_ITER_NUM)->GetWindowTextW(strTemp,128);
		m_ClassifierParam.iMaxIter=_wtoi(strTemp);
		...
		WideCharToMultiByte(CP_ACP, 0, strClassifierFileName, -1, m_ClassifierParam.fileName, iLength, NULL, NULL);
		//ImgProcessClassifier::Classifier_SVM(trainData,trainLabels,m_ClassIndexNum,m_ClassifierParam);
		H_ClassifierTrain("SVM分类器",trainData,trainLabels,m_ClassIndexNum,m_ClassifierParam);
		strInfo.Format(L"SVM分类器：迭代次数为%d，结果精度为%f，存储文件名为%s",m_ClassifierParam.iMaxIter,m_ClassifierParam.fEpsilon,strClassifierFileName);
		m_ListShowInfo.InsertString(g_lInsertIndex++,strInfo);
		m_ListShowInfo.InsertString(g_lInsertIndex++,L"测试中...");
		H_ClassifierPredict("SVM分类器",testData,testLabels,perLabels,m_ClassIndexNum,m_ClassifierParam);
		break;
	default:break;
	}
	///*****************************训练完成*******************************///
...
void DlgClassifier::WriteClassifierParamToIniFile()
{
	...
	USES_CONVERSION;
    strValue = A2T(m_ClassifierParam.fileName);
...
void DlgClassifier::WriteResultToFile(SampleImgSet ImgSet,cv::Mat iLabelClass,cv::Mat iPerClass)
{
	...
	for(int i=0;i<ImgSet.iItemNum;i++)
	{
		...
		USES_CONVERSION;
		CString strClassName = A2T(CArNTClassifierConsoleDlg::m_ConfigClassSet.Items[iInterClass].strClassName);
...
///显示混淆矩阵
void DlgClassifier::ShowConfusionMatrix(const Mat& confusion_matrix)
{
	CPen newPen;       // 用于创建新画笔   
