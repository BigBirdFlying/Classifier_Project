...
void DlgSampleCollection::OnPaint()
{
	...
	pDC.StretchBlt(10,10,100,100,&dcMemory,0,0,300,300,SRCCOPY);  
...
void DlgSampleCollection::OnBnClickedButtonAutoseg()
{
	...
	if(m_grayImage.rows>0 && m_grayImage.cols>0)
	{
		...
		//ConfigurationInfo tConfigurationInfo={0};
		///分割Dll文件接口
		CString strSelect;   
		...
		H_DefectDetectInterfaceFunc(iProductLine,m_grayImage.data,m_grayImage.cols,m_grayImage.rows,m_ConfigurationInfo,pH_DefectSet);
		///获得缺陷区域图像块
		for(int n=0;n<pH_DefectSet->iItemNum;n++)
		{
			...
			cv::rectangle(m_colorImage,cv::Point(rect.tl().x,rect.tl().y),cv::Point(rect.br().x,rect.br().y),Scalar(0,255,255),2);
			cv::putText(m_colorImage, strnum, Point((rect.tl().x+rect.br().x)/2,(rect.tl().y+rect.br().y)/2), FONT_HERSHEY_PLAIN, 2, Scalar( 0, 0, 255), 2);
			//获得的图像块放到结构体
